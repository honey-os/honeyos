...
interface SessionsListProps {
  /** Protocol from the URL path segment, e.g. /sessions/type/rdp */
  pathProtocol?: string;
}

export default function SessionsList({ pathProtocol }: SessionsListProps) {
  const {
    sessions,
    sessionsTotal,
    sessionsPage,
  ...
  // Path-based protocol takes precedence over query param
  const filterProtocol = pathProtocol || getParam('protocol');
  ...
  const handleProtocolChange = (value: string) => {
    if (value) {
      const statusQs = filterStatus ? `?status=${filterStatus}` : '';
      router.push(`/sessions/type/${value}${statusQs}`);
    } else {
      const statusQs = filterStatus ? `?status=${filterStatus}` : '';
      router.push(`/sessions${statusQs}`);
    }
  };

  const handleStatusChange = (value: string) => {
    if (pathProtocol) {
      const statusQs = value ? `?status=${value}` : '';
      router.push(`/sessions/type/${pathProtocol}${statusQs}`);
    } else {
